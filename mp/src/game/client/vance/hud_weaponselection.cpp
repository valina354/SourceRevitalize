...
private:
	...
	CPanelAnimationVar(vgui::HFont, m_hNumberFontGlow, "NumberFont", "HudSelectionNumbersGlow");
	CPanelAnimationVar(vgui::HFont, m_hTextFont, "TextFont", "HudSelectionText");
	CPanelAnimationVar(vgui::HFont, m_hLabelFont, "LabelFont", "HudSelect")
		CPanelAnimationVar(vgui::HFont, m_hLabelGlowFont, "LabelGlowFont", "HudSelectGlow")
		CPanelAnimationVar(vgui::HFont, m_hWeaponGlowFont, "WeaponGlowFont", "WeaponGlowIcons")
		CPanelAnimationVar(float, m_flBlur, "Blur", "0");

	CPanelAnimationVarAliasType(float, m_flSmallBoxSize, "SmallBoxSize", "8", "proportional_float");
	CPanelAnimationVarAliasType(float, m_flLargeBoxWide, "LargeBoxWide", "18", "proportional_float");
	CPanelAnimationVarAliasType(float, m_flLargeBoxTall, "LargeBoxTall", "12", "proportional_float");
...
void CHudWeaponSelection::Paint()
{
	...
	for (int i = 0; i < MAX_WEAPON_SLOTS; i++)
	{
		if (i == iActiveSlot)
		{
			...
			for (int slotpos = -1; slotpos <= iLastPos; slotpos++)
			{

				C_BaseCombatWeapon *pWeapon = GetWeaponInSlot(i, slotpos);
				if (!pWeapon && slotpos >= 0)
				{
					if (!hud_showemptyweaponslots.GetBool())
						continue;
					DrawBox(xpos, ypos, largeBoxWide, largeBoxTall, m_BoxColor, m_BoxColor.a(), bDrawBucketNumber ? i + 1 : -1);
				}
				else if (slotpos == -1)
				{
					DrawBox(xpos, ypos, m_flSmallBoxSize, m_flSmallBoxSize, m_BoxColor, m_BoxColor.a(), bDrawBucketNumber ? i + 1 : -1);
			...
			if (GetFirstPos(i))
			{
				...
				for (int slotpos = -1; slotpos <= iLastPos; slotpos++)
				{
					if (slotpos == -1)
					{
						...
						DrawBox(xpos, ypos, m_flSmallBoxSize, m_flSmallBoxSize, m_BoxColor, (m_flAlphaOverride * 1.15f), -1);
					}
					if (slotpos >= 0)
					{
...
void CHudWeaponSelection::DrawLargeWeaponBox(C_BaseCombatWeapon* pWeapon, bool bSelected, int xpos, int ypos, int boxWide, int boxTall, Color selectedColor, float alpha, int number)
{
	...
	if (pWeapon->GetSpriteActive())
	{
		...
		wchar_t unicode[8];
		V_snwprintf(unicode, ARRAYSIZE(unicode), L"%c", pWeapon->GetSpriteActive()->cCharacterInFont);
		surface()->DrawSetTextFont(m_hWeaponGlowFont);
		surface()->DrawSetTextColor(Color(255, 0, 0, 255));
		surface()->DrawSetTextPos(xpos + x_offs, ypos + y_offs);
...
void CHudWeaponSelection::DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, int number)
{
	...
	if (number >= 0)
	{
		...
		surface()->DrawSetTextFont(m_hNumberFontGlow);
		surface()->DrawSetTextPos(x + m_flSelectionNumberXPos, y + m_flSelectionNumberYPos);
		surface()->DrawUnicodeChar(wch);
