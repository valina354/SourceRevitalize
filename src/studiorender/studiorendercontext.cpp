											...
											OptimizedModel::StripGroupHeader_t *pStripGroup )
{
	...
	int i;
	int stripDataSize = 0;
	size_t stripHdrSize = ( pStripGroup->flags & OptimizedModel::STRIPGROUP_IS_MDL49 ) ? sizeof( OptimizedModel::StripHeader_v49_t ) : sizeof( OptimizedModel::StripHeader_t );
	for( i = 0; i < pStripGroup->numStrips; ++i )
	{
	...
	for( i = 0; i < pStripGroup->numStrips; ++i )
	{
		...
		pMeshGroup->m_pStripData[i].boneStateChangeOffset = boneStateChangeOffset - i * stripHdrSize;
