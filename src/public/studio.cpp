...
const char *studiohdr_t::pszNodeName( int iNode )
{
	...
	return pVModel->m_group[pVModel->m_node[iNode - 1].group].GetStudioHdr()->pszLocalNodeName(
		pVModel->m_node[iNode - 1].index );
}
...
const studiohdr_t *CStudioHdr::GroupStudioHdr( int i )
{
	...
	if ( !m_pStudioHdrCache.IsValidIndex( i ) )
	{
		...
		ExecuteNTimes( 5,
					   Warning( "Invalid index passed to CStudioHdr(%s)::GroupStudioHdr(): %d, but max is %d\n", pszName, i,
								m_pStudioHdrCache.Count() ) );
...
void CStudioHdr::CActivityToSequenceMapping::Initialize( CStudioHdr *__restrict pstudiohdr )
{
	...
	for ( UtlHashHandle_t handle = m_ActToSeqHash.GetFirstHandle(); m_ActToSeqHash.IsValidHandle( handle );
	...
	for ( int i = 0; i < NumSeq; ++i )
	{
		...
		if ( seqdesc.activity >= 0 )
		{
			...
			if ( seqdesc.numactivitymodifiers > 0 )
			{
				...
				{
					( tupleList + element.startingIdx + tupleOffset )->pActivityModifiers[k] =
						g_ActivityModifiersTable.AddString( seqdesc.pActivityModifier( k )->pszName() );
				}
			}
			else
			{
