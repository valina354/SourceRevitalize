...
inline void CalcBoneQuaternion( int frame, float s, const mstudiobone_t *pBone, const mstudiolinearbone_t *pLinearBones, const mstudioanim_t *panim, Quaternion &q )
{
	if ( pLinearBones )
	{
		CalcBoneQuaternion( frame, s, pLinearBones->quat( panim->bone ), pLinearBones->rot( panim->bone ),
							pLinearBones->rotscale( panim->bone ), pLinearBones->flags( panim->bone ),
							pLinearBones->qalignment( panim->bone ), panim, q );
	}
	else
	{
	...
	if ( animdesc.numlocalhierarchy )
	{
		...
		for ( int l = 0; l < animdesc.numlocalhierarchy; l++ )
		{
			...
			if ( pStudioHdr->boneFlags( pHierarchy->iBone ) & boneMask )
			{
				...
				{
					CalcLocalHierarchyAnimation( pStudioHdr, boneToWorld, boneComputed, pos, q, pbone, pHierarchy,
												 pHierarchy->iBone, pHierarchy->iNewParent, cycle, iFrame, s, boneMask );
				}
			}
		}
...
bool Studio_IKSequenceError( const CStudioHdr *pStudioHdr, mstudioseqdesc_t &seqdesc, int iSequence, float flCycle, int iRule, const float poseParameter[], mstudioanimdesc_t *panim[4], float weight[4], ikcontextikrule_t &ikRule )
{
	...
	if ( ikRule.flWeight <= 0.001f )
	{
		...
		if ( panim[0]->pIKRule( iRule ) == NULL )
			return false;
		if ( ( panim[0]->flags & STUDIO_LOOPING ) && panim[0]->pIKRule( iRule )->type == IK_GROUND &&
			...
			if ( pTarget->error.ramp > 0.0 )
			{
				...
				else
				{
					DevWarning( 1, "AutoIKRelease (%s) got an out of range chain %d (%d)\n", m_pStudioHdr->pszName(),
								pTarget->chain, m_pStudioHdr->numikchains() );
				}
			}
			else
			{
...
void CIKContext::SolveLock( const mstudioiklock_t *plock, int i, Vector pos[], Quaternion q[], matrix3x4_t boneToWorld[], CBoneBitList &boneComputed )
{
	...
	{
		Studio_SolveIK( pchain->pLink( 0 )->bone, pchain->pLink( 1 )->bone, pchain->pLink( 2 )->bone, p3, m_ikLock[i].kneePos,
						m_ikLock[i].kneeDir, boneToWorld );
	}
	else
	{
		Studio_SolveIK( pchain, p3, boneToWorld );
	...
	for ( int i = 0; i < nBoneFlexDriverCount; ++i )
	{
		...
		if ( pStudioBone->flags & BONE_USED_BY_BONE_MERGE )
		{
			...
			for ( int j = 0; j < nControllerCount; ++j )
			{
				...
				const mstudioflexcontroller_t *pFlexController =
					pStudioHdr->pFlexcontroller( static_cast<LocalFlexController_t>( pController->m_nFlexControllerIndex ) );
