...
void CScratchPadGraph::Init( IScratchPad3D *pPad,
							 ...
							 Vector vValueAxis, float flInchesPerValue, Vector vValueLineColor, float flValueOrigin
...
void CScratchPadGraph::AddSample( LineID iLine, float flTime, float flValue )
{
	...
	if ( !pInfo->m_bFirst )
	{
		...
		m_pPad->DrawLine( CSPVert( vStart, pInfo->m_vColor ), CSPVert( vEnd, pInfo->m_vColor ) );
	}
...
void CScratchPadGraph::UpdateTicksAndStuff( float flTime, float flValue )
{
	...
	if ( flValue > m_flHighestValue )
	{
		...
		for ( int i = 0; i < m_nTimeLabelsDrawn; i++ )
		{
			...
			m_pPad->DrawLine( CSPVert( (const Vector &)GetSamplePosition( flTimeLocl, m_flHighestValue ) ), CSPVert( (const Vector &)GetSamplePosition( flTimeLocl, flValue ) ) );
		}
	...
	while ( m_nTimeLabelsDrawn < iHighestTextLabel )
	{
		...
		m_pPad->DrawLine( CSPVert( (const Vector &)GetSamplePosition( flTimeLocl, m_flValueOrigin ) ), CSPVert( (const Vector &)GetSamplePosition( flTimeLocl, m_flHighestValue ) ) );
...
{
	Vector vRet = m_vTimeAxis * ( ( flTime - m_flTimeOrigin ) * m_flInchesPerSecond ) + m_vValueAxis * ( ( flValue - m_flValueOrigin ) * m_flInchesPerValue );
...
void ScratchPad_DrawLitCone( IScratchPad3D *pPad, const Vector &vBaseCenter, const Vector &vTip, const Vector &vBrightColor, const Vector &vDarkColor, const Vector &vLightDir, float baseWidth, int nSegments )
{
...
void ScratchPad_DrawLitCylinder( IScratchPad3D *pPad, const Vector &v1, const Vector &v2, const Vector &vBrightColor, const Vector &vDarkColor, const Vector &vLightDir, float width, int nSegments )
{
	// Make orthogonal vectors.
	Vector vDir = v2 - v1;
	VectorNormalize( vDir );
...
void ScratchPad_DrawArrow( IScratchPad3D *pPad, const Vector &vPos, const Vector &vDirection, const Vector &vColor, float flLength, float flLineWidth, float flHeadWidth, int nCylinderSegments, int nHeadSegments, float flArrowHeadPercentage )
{
...
void ScratchPad_DrawArrowSimple( IScratchPad3D *pPad, const Vector &vPos, const Vector &vDirection, const Vector &vColor, float flLength )
{
	ScratchPad_DrawArrow( pPad, vPos, vDirection, vColor, flLength, flLength * 1.0 / 15, flLength * 3.0 / 15, 4, 4 );
}

void ScratchPad_DrawSphere( IScratchPad3D *pPad, const Vector &vCenter, float flRadius, const Vector &vColor, int nSubDivs )
{
...
void ScratchPad_DrawAABB( IScratchPad3D *pPad, const Vector &vMins, const Vector &vMaxs, const Vector &vColor )
{
