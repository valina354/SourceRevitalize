...
//-----------------------------------------------------------------------------
class CGGrabController : public IMotionEvent
{
	DECLARE_SIMPLE_DATADESC();

public:

	CGGrabController(void);
	~CGGrabController(void);
...
BEGIN_SIMPLE_DATADESC(CGGrabController)
...
CGGrabController::CGGrabController(void)
{
...
void CGGrabController::OnRestore()
{
	if (m_controller)
	{
...
void CGGrabController::SetTargetPosition(const Vector &target, const QAngle &targetOrientation)
{
...
// Output : float
//-----------------------------------------------------------------------------
float CGGrabController::ComputeError()
{
	if (m_errorTime <= 0)
		return 0;
...
void CGGrabController::ComputeMaxSpeed(CBaseEntity *pEntity, IPhysicsObject *pPhysics)
{
	m_shadow.maxSpeed = 1000;
	m_shadow.maxAngular = DEFAULT_MAX_ANGULAR;
...
QAngle CGGrabController::TransformAnglesToPlayerSpace(const QAngle &anglesIn, CBasePlayer *pPlayer)
{
...
QAngle CGGrabController::TransformAnglesFromPlayerSpace(const QAngle &anglesIn, CBasePlayer *pPlayer)
{
	if (m_bIgnoreRelativePitch)
	{
		matrix3x4_t test;
...
void CGGrabController::AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, bool bIsMegaPhysCannon, const Vector &vGrabPosition, bool bUseGrabPosition)
{
...
void CGGrabController::DetachEntity(bool bClearVelocity)
{
...
IMotionEvent::simresult_e CGGrabController::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular)
{
...
float CGGrabController::GetSavedMass(IPhysicsObject *pObject)
{
...
//-----------------------------------------------------------------------------
bool CGGrabController::IsObjectAllowedOverhead(CBaseEntity *pEntity)
{
...
public:
	...
	CGGrabController &GetGrabController() { return m_grabController; }

private:
	CGGrabController		m_grabController;
...
public:
	...
	//it, but fuck you this is easier.
	virtual bool	Reload();
...
bool CWeaponPhysCannon::Reload() {
	CVancePlayer *pPlayer = static_cast<CVancePlayer *>( GetOwner() );
	if (pPlayer)	{
		if (!m_bActive) {
			pPlayer->m_bGglovesManipulatorHasTarget = false;
			...
			{
				pConeEntity =
					MegaPhysCannonFindObjectInCone( start, forward, physcannon_cone.GetFloat(), physcannon_ball_cone.GetFloat(), bAttach || bPull );
			}
			...
			if (pObj->GetMass() != REDUCED_CARRY_MASS) {
				pPlayer->m_fGglovesMainpulatorTargetMass = pObj->GetMass();
			}

			if ( bAttach )
			{
				pPlayer->m_bGglovesManipulatorHasTarget = true;
				return false;
			}
			...
			pPlayer->m_bGglovesManipulatorHasTarget = true;
			pPlayer->m_fGglovesMainpulatorTargetMass = 0;
			for (int i = 0; i < VPHYSICS_MAX_OBJECT_LIST_COUNT; i++) {
				pPlayer->m_fGglovesMainpulatorTargetMass += m_grabController.m_savedMass[i];
			}
		}
	}
	return false;
}
void CWeaponPhysCannon::WeaponSound(WeaponSound_t sound_type, float soundtime /* = 0.0f */) {
	//best as i can tell this function wont work because the weapon isnt assigned properly, so im just gonna hard code it.
	const char *shootsound;
	switch (sound_type)
	{
	case SINGLE:
		shootsound = "Weapon_PhysCannon.Launch";
		break;
	case EMPTY:
		shootsound = "Weapon_PhysCannon.DryFire";
		break;
	case SPECIAL1:
		shootsound = "Weapon_PhysCannon.Pickup";
		break;
	case MELEE_MISS:
		shootsound = "Weapon_PhysCannon.Drop";
		break;
	case SPECIAL3:
		shootsound = "Weapon_PhysCannon.TooHeavy";
	default:
		return;
	}
	ToBasePlayer(GetOwner())->EmitSound(shootsound);
}
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CGGrabController::UpdateObject(CBasePlayer *pPlayer, float flError)
{
	CBaseEntity *pEntity = GetAttached();
...
void CWeaponPhysCannon::ItemPreFrame()
{
	...
	if (m_bActive) {
		pVanceOwner->m_bGglovesManipulatorHasAttached = true;
	} else {
		pVanceOwner->m_bGglovesManipulatorHasAttached = false;
	}
#endif
}
...
float PhysCannonGetHeldObjectMass(CBaseCombatWeapon *pActiveWeapon, IPhysicsObject *pHeldObject)
{
	...
	if (pCannon)
	{
		CGGrabController &grab = pCannon->GetGrabController();
...
float PlayerPickupGetHeldObjectMass(CBaseEntity *pPickupControllerEntity, IPhysicsObject *pHeldObject)
{
	...
	if (pController)
	{
		CGGrabController &grab = pController->GetGrabController();
		mass = grab.GetSavedMass(pHeldObject);
	}
	return mass;
}
