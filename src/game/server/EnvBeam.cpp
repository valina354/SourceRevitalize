	...
	if ( pointStart || pointEnd )
	{
		...
		if ( m_spawnflags & SF_BEAM_RING )
		{
			te->BeamRing( filter, 0.0, pStart->entindex(), pEnd->entindex(), m_spriteTexture,
						  0, // No halo
						  m_frameStart, (int)m_flFrameRate, m_life, m_boltWidth,
						  0, // No spread
						  m_noiseAmplitude, m_clrRender->r, m_clrRender->g, m_clrRender->b, m_clrRender->a, m_speed );
		}
		else
		{
			te->BeamEntPoint( filter, 0.0, pStart->entindex(), &pStart->GetAbsOrigin(), pEnd->entindex(), &pEnd->GetAbsOrigin(), m_spriteTexture,
							  0, // No halo
							  m_frameStart, (int)m_flFrameRate, m_life, m_boltWidth,
