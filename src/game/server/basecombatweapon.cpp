...
void CBaseCombatWeapon::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	if ( (pEvent->type & AE_TYPE_NEWEVENTSYSTEM) && (pEvent->type & AE_TYPE_SERVER) )
	{
		...
		else if (pEvent->event == AE_WEAPON_ALLOWFIRE)
		{
			...
			//if we are sprinting do the sprint anim
			CBasePlayer* pPlayer = ToBasePlayer(GetOwner());
			if (pPlayer && pPlayer->GetLocalVelocity().Length2D() > 300 && (GetActivity() == ACT_VM_RELOAD || GetActivity() == ACT_VM_RELOAD_EXTENDED || GetActivity() == ACT_VM_DRAW || GetActivity() == ACT_VM_DRAW_EXTENDED))
			{
