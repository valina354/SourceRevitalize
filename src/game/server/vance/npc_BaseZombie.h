...
enum 
{
	...
	TASK_ZOMBIE_PLAY_STAND_UP_SEQUENCE,
...
protected:
	...
	void TempRagdoll_Become(const CTakeDamageInfo& info, bool bUseLRURetirement = true); // become temporary ragdoll
	void TempRagdoll_Hide(); // hide ourselves when turning into a temporary ragdoll
	void TempRagdoll_Think(); // count some time until we can become npc again
	int  TempRagdoll_GetStandUpActivity();

	CHandle<CRagdollProp>m_hTemp_Ragdoll;
	int					 m_iTemp_MoveType;
	int					 m_iTemp_StandUpActivity;
	bool				 m_bIsTempRagdoll;
