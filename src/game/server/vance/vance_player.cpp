...
ConVar vance_kick_firerate("kick_firerate", "0.7", FCVAR_CHEAT);
ConVar vance_kick_damage_mult_min("kick_damage_mult_min", "1.2", FCVAR_CHEAT);
ConVar vance_kick_damage_mult_max("kick_damage_mult_max", "2.5", FCVAR_CHEAT);
...
ConVar vance_kick_knockback_scale("kick_knockback_scale", "1.2", FCVAR_CHEAT);
...
IMPLEMENT_SERVERCLASS_ST( CVancePlayer, DT_Vance_Player )
	...
	SendPropBool(SENDINFO(m_bOneHandedWeapon))
END_SEND_TABLE()
...
{
	CVancePlayer *pPlayer = static_cast<CVancePlayer *>(UTIL_GetCommandClient());
	if (pPlayer && pPlayer->HasNamedPlayerItem("weapon_unarmed"))
	{
		if (strcmp(pPlayer->GetActiveWeapon()->GetClassname(), "weapon_unarmed") == 0) {
			...
			CBaseCombatWeapon* unarmedweapon = static_cast<CBaseCombatWeapon*>(pPlayer->HasNamedPlayerItem("weapon_unarmed"));
			if (unarmedweapon)
				pPlayer->GetActiveWeapon()->Holster(unarmedweapon);
		}
	}
}
...
void CVancePlayer::Precache()
{
	...
	PrecacheModel(V_ONEHANDEDWEAPON);

	PrecacheModel( "models/weapons/v_stim.mdl" );

	PrecacheModel("models/weapons/v_gravgloves.mdl");
...
void CVancePlayer::EquipSuit(bool bPlayEffects)
{
	...
	if (IsSuitEquipped() != bHadSuit)
	{
		...
		if (pLeg) {
			...
			pLeg->SendViewModelMatchingSequence(pLeg->SelectWeightedSequence(ACT_VM_IDLE));
		}
		if ( CBaseViewModel *pOneHandedGunViewModel = GetViewModel( VM_ONEHANDEDGUN ) ) {
			pOneHandedGunViewModel->SetWeaponModel( V_ONEHANDEDWEAPON, NULL );
			pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_HOLSTER));
		}
	}
...
{
	//one handed alygun, for use in vehicles and while grav gloving
	if (m_bOneHandedWeapon.Get() && HasNamedPlayerItem("weapon_resistancegun") && GetViewModel(VM_ONEHANDEDGUN)) {
		CBaseViewModel *pOneHandedGunViewModel = GetViewModel(VM_ONEHANDEDGUN);
		CBaseCombatWeapon* alyxgun = static_cast<CBaseCombatWeapon*>(HasNamedPlayerItem("weapon_resistancegun"));

		if (!m_bOneHandedWeaponWasActive) {
			//draw
			pOneHandedGunViewModel->ClearEffects();
			pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_DRAW));
			m_bOneHandedWeaponSprintStage = 0;
			m_fOneHandedWeaponIdleTime = gpGlobals->curtime + pOneHandedGunViewModel->SequenceDuration();
			alyxgun->m_flNextPrimaryAttack  = gpGlobals->curtime  + pOneHandedGunViewModel->SequenceDuration();

			m_bOneHandedWeaponWasActive = true;
		}

		if (m_fOneHandedWeaponIdleTime <= gpGlobals->curtime) {
			float playerSpeed = GetLocalVelocity().Length2D();
			if (IsSprinting() && playerSpeed >= 300) {
				if (m_bOneHandedWeaponSprintStage == 0 && pOneHandedGunViewModel->GetSequenceActivity(pOneHandedGunViewModel->GetSequence()) != ACT_VM_SPRINT_ENTER) {
					pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_SPRINT_ENTER));
					m_bOneHandedWeaponSprintStage = 1;
					m_fOneHandedWeaponSprintIntroEndTime = gpGlobals->curtime + pOneHandedGunViewModel->SequenceDuration();
				} else if (m_bOneHandedWeaponSprintStage == 1 && m_fOneHandedWeaponSprintIntroEndTime <= gpGlobals->curtime && pOneHandedGunViewModel->GetSequenceActivity(pOneHandedGunViewModel->GetSequence()) != ACT_VM_SPRINT) {
					pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_SPRINT));
				}
			} else {
				if ((pOneHandedGunViewModel->GetSequenceActivity(pOneHandedGunViewModel->GetSequence()) == ACT_VM_SPRINT_ENTER 
				|| pOneHandedGunViewModel->GetSequenceActivity(pOneHandedGunViewModel->GetSequence()) == ACT_VM_SPRINT)
				&& pOneHandedGunViewModel->GetSequenceActivity(pOneHandedGunViewModel->GetSequence()) != ACT_VM_SPRINT_LEAVE) {
					pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_SPRINT_LEAVE));
					m_bOneHandedWeaponSprintStage = 0;
					m_fOneHandedWeaponSprintIntroEndTime = gpGlobals->curtime + pOneHandedGunViewModel->SequenceDuration();
				} else if(pOneHandedGunViewModel->GetSequenceActivity(pOneHandedGunViewModel->GetSequence()) != ACT_VM_IDLE && m_fOneHandedWeaponSprintIntroEndTime <= gpGlobals->curtime) {
					pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_IDLE));
					m_bOneHandedWeaponSprintStage = 0;
				}
			}
		}
		if (m_nButtons & IN_ATTACK && alyxgun->m_flNextPrimaryAttack <= gpGlobals->curtime) {
			if (alyxgun->m_iClip1 > 0) {
				pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_PRIMARYATTACK));
				m_fOneHandedWeaponIdleTime = gpGlobals->curtime + pOneHandedGunViewModel->SequenceDuration();
				m_bOneHandedWeaponSprintStage = 0;
			}
			alyxgun->PrimaryAttack();
		}
		if ((alyxgun->m_iClip1 <= 0 || m_afButtonPressed & IN_RELOAD) && alyxgun->m_iClip1 < alyxgun->GetMaxClip1() && !m_bOneHandedWeaponInReload && alyxgun->m_flNextPrimaryAttack <= gpGlobals->curtime) {
			//if we dont have any more ammo then we arent allowed to have the onehanded alyxgun out anymore
			if (GetAmmoCount(alyxgun->GetPrimaryAmmoType()) <= 0 && alyxgun->m_iClip1 <= 0) {
				m_bOneHandedWeapon = false;
			} else {
				pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_RELOAD));
				m_bOneHandedWeaponSprintStage = 0;
				m_fOneHandedWeaponIdleTime = gpGlobals->curtime + pOneHandedGunViewModel->SequenceDuration();
				alyxgun->m_flNextPrimaryAttack  = gpGlobals->curtime  + pOneHandedGunViewModel->SequenceDuration();
				m_fOneHandedWeaponFinishReloadTime = gpGlobals->curtime + 1.5f;
				m_bOneHandedWeaponInReload = true;
			}
		}
		if (m_bOneHandedWeaponInReload && m_fOneHandedWeaponFinishReloadTime <= gpGlobals->curtime) {
			//refill ammo
			int fillammount = MIN((alyxgun->GetMaxClip1() - alyxgun->m_iClip1), GetAmmoCount(alyxgun->GetPrimaryAmmoType()));
			RemoveAmmo(fillammount, alyxgun->GetPrimaryAmmoType());
			alyxgun->m_iClip1 += fillammount;
			m_bOneHandedWeaponInReload = false;
		}
	} else if (m_bOneHandedWeaponWasActive) {
		if (GetViewModel(VM_ONEHANDEDGUN)) {
			CBaseViewModel *pOneHandedGunViewModel = GetViewModel(VM_ONEHANDEDGUN);
			//holster
			pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_HOLSTER));
			m_bOneHandedWeaponSprintStage = 0;
			m_fOneHandedWeaponDisapearTime = gpGlobals->curtime + pOneHandedGunViewModel->SequenceDuration();
		}
		m_bOneHandedWeaponWasActive = false;
	} else if (m_fOneHandedWeaponDisapearTime <= gpGlobals->curtime && GetViewModel(VM_ONEHANDEDGUN)) {
		CBaseViewModel *pOneHandedGunViewModel = GetViewModel(VM_ONEHANDEDGUN);
		pOneHandedGunViewModel->SetEffects(EF_NODRAW);
	}
	...
	if (m_eGglovesManipulator) {
		//zap the gravity gun to life
		CBaseHLCombatWeapon* physgunent = static_cast<CBaseHLCombatWeapon*>(m_eGglovesManipulator);
		if (physgunent) {
			physgunent->ItemPreFrame();
		}
	}
...
int CVancePlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
	...
	else
	{
		...
		if ( !bTrivial && m_flNextPainSound < gpGlobals->curtime )
		{
			...
			m_flNextPainSound = gpGlobals->curtime + RandomFloat( 2.0f, 4.0f );
		}
	}
...
//give the 'gloves' which is just a gravity gun that doesnt have a viewmodel (the same as use key)
void CVancePlayer::SetupGravityGloves() {
	CBaseHLCombatWeapon* physgunent = static_cast<CBaseHLCombatWeapon*>(CreateEntityByName("weapon_physcannon"));
	physgunent->SetOwner(this);
	m_eGglovesManipulator = physgunent;
	//parent it to us so we get the hold_loop audio in the right place
	physgunent->SetParent(this);
	physgunent->SetLocalOrigin(Vector(0, 0, 0));
}

void CVancePlayer::StartGravGloving() {
	//need the suit for this
	if (!IsSuitEquipped())
		return;
	...
	//physgun should have an actual target in mind here, since we are reusing the e key
	physgunent->Reload(); //updates m_bGglovesManipulatorHasTarget
	if (!m_bGglovesManipulatorHasTarget)
		return;

	// Player must not be performing an action
	if (m_PerformingGesture == GestureAction::None) {
		m_PerformingGesture = GestureAction::GravGloving;
	} else {
		return;
	}
	...
	// figure out with the use key system what all we should be doing (or maybe that should be what actually calls this function)

	CVanceViewModel *pViewModel = static_cast<CVanceViewModel *>(GetViewModel());
	if (pViewModel)
	{
		pViewModel->SetWeaponModel("models/weapons/v_gravgloves.mdl", nullptr);

		int idealSequence = pViewModel->SelectWeightedSequence(RemapGravGloveActivities(ACT_VM_DRAW_EXTENDED));
		if (idealSequence >= 0)
		{
			pViewModel->SendViewModelMatchingSequence(idealSequence);
			m_fGglovesStartIdleTime = gpGlobals->curtime + pViewModel->SequenceDuration();
			m_fGestureFinishTime = gpGlobals->curtime + pViewModel->SequenceDuration() + 0.5f;
		} else {
			Msg("no sequence found");
			return;
		}
	}

	m_fGglovesPuntTime = 0.0f;
	m_bGglovesWasActive = false;
	m_fGglovesTimeout = gpGlobals->curtime + 0.5f;
	m_bGglovesShouldntUpdate = false;
	m_bGglovesAlreadyPunted = false;
...
void CVancePlayer::GravGlovingTick() {
	CVanceViewModel *pViewModel = static_cast<CVanceViewModel *>(GetViewModel());
	CBaseHLCombatWeapon *physgunent = static_cast<CBaseHLCombatWeapon *>( m_eGglovesManipulator );
	if (!pViewModel || !physgunent)
		return;
	if (strcmp(pViewModel->GetModelName().ToCStr(), "models/weapons/v_gravgloves.mdl")) { //stop weapon switching from fucking stuff up
		pViewModel->SetWeaponModel("models/weapons/v_gravgloves.mdl", nullptr);
		m_fGglovesStartIdleTime = 0;
	}

	if (pViewModel->GetSequenceActivity(pViewModel->GetSequence()) == RemapGravGloveActivities(ACT_VM_FIRE_EXTENDED) && !m_bGglovesAlreadyPunted) {
		if (gpGlobals->curtime >= m_fGglovesPuntTime) {
			physgunent->PrimaryAttack();
			m_bGglovesAlreadyPunted = true;
			m_flNextSprint = gpGlobals->curtime + sk_nextsprint_delay.GetFloat();
		}
		return;
	}

	if (pViewModel->GetSequenceActivity(pViewModel->GetSequence()) == RemapGravGloveActivities(ACT_VM_HOLSTER_EXTENDED)) {
		if (m_bGglovesManipulatorHasAttached) {
			physgunent->SecondaryAttack();
		}
		return;
	}

	//get new physcannon data
	if (!m_bGglovesShouldntUpdate)
		physgunent->Reload();

	// punt
	if (m_bGglovesManipulatorHasAttached && (m_afButtonPressed & IN_ATTACK2)) {
		pViewModel->SetSequence(pViewModel->SelectWeightedSequence(RemapGravGloveActivities(ACT_VM_FIRE_EXTENDED)));
		if (GetActiveWeapon())
			GetActiveWeapon()->m_fDoNotDisturb = gpGlobals->curtime + pViewModel->SequenceDuration();
		m_fGestureFinishTime = gpGlobals->curtime + pViewModel->SequenceDuration();
		m_fGglovesStartIdleTime = gpGlobals->curtime + pViewModel->SequenceDuration();
		if (ShouldGravGlovesOnehand()) {
			m_fGglovesPuntTime = gpGlobals->curtime + 0.05;
		} else {
			m_fGglovesPuntTime = gpGlobals->curtime + 0.23f;
		}
		m_bGglovesShouldntUpdate = true;
		m_bOneHandedWeapon = false;
		m_vGglovesManipulatorOffset = false;
		return;
	}

	// if we are pulling then make sure we still have something to pull, we are gonna give a second leeway here cause its easy to lose distance objects
	if (!m_bGglovesManipulatorHasAttached && m_bGglovesManipulatorHasTarget) {
		//we have a target, reset the timeout
		m_fGglovesTimeout = gpGlobals->curtime + 0.5f;
	}

	// player manually cancles
	if ((m_bGglovesManipulatorHasAttached && (m_afButtonPressed & IN_USE)) || (!m_bGglovesManipulatorHasAttached && !(m_nButtons & IN_USE)) || (!m_bGglovesManipulatorHasAttached && m_fGglovesTimeout <= gpGlobals->curtime)) {
		pViewModel->SetSequence(pViewModel->SelectWeightedSequence(RemapGravGloveActivities(ACT_VM_HOLSTER_EXTENDED)));
		if (GetActiveWeapon())
			GetActiveWeapon()->m_fDoNotDisturb = gpGlobals->curtime + pViewModel->SequenceDuration();
		if (m_bGglovesManipulatorHasAttached) {
			physgunent->SecondaryAttack();
	...
	//keep phys pulling if we dont have anything active yet
	if (!m_bGglovesManipulatorHasAttached) {
		physgunent->SecondaryAttack();
		if (ShouldGravGlovesOnehand()) {
			m_vGglovesManipulatorOffset = true;
		} else {
			m_vGglovesManipulatorOffset = false;
		}
	}

	//do the funny anim when item is picked up
	if (!m_bGglovesWasActive && m_bGglovesManipulatorHasAttached) {
		if (pViewModel->GetSequenceActivity(pViewModel->GetSequence()) != RemapGravGloveActivities(ACT_VM_DRAW_EXTENDED)) {
			pViewModel->SetSequence(pViewModel->SelectWeightedSequence(RemapGravGloveActivities(ACT_VM_RECOIL2)));
			m_fGglovesStartIdleTime = gpGlobals->curtime + pViewModel->SequenceDuration() - 0.1f;
			m_fGestureFinishTime = gpGlobals->curtime + pViewModel->SequenceDuration() + 0.5f;
		}
		if (m_nButtons & IN_SPEED) {
			StartSprinting(); //gravity gun debounces sprint cause its lame and stupid
		}
		m_bGglovesWasActive = true;
		if (ShouldGravGlovesOnehand()) {
			m_bOneHandedWeapon = true;
			m_vGglovesManipulatorOffset = true;
		} else {
			m_vGglovesManipulatorOffset = false;
		}
	...
	//keep idling
	if (gpGlobals->curtime >= m_fGglovesStartIdleTime) {
		if (m_bGglovesManipulatorHasAttached) {
			pViewModel->SetSequence(pViewModel->SelectWeightedSequence(RemapGravGloveActivities(ACT_VM_IDLE_EXTENDED)));
		} else {
			pViewModel->SetSequence(pViewModel->SelectWeightedSequence(RemapGravGloveActivities(ACT_VM_RECOIL1)));
		}
		m_fGglovesStartIdleTime = gpGlobals->curtime + pViewModel->SequenceDuration() - 0.1f;
		m_fGestureFinishTime = gpGlobals->curtime + pViewModel->SequenceDuration() + 0.5f;
	}
	...
	if (GetActiveWeapon())
	{
		GetActiveWeapon()->m_fDoNotDisturb = gpGlobals->curtime + 0.5f;
		GetActiveWeapon()->AbortReload();
	}
}

Activity CVancePlayer::RemapGravGloveActivities(Activity twohandedact) {
	if (ShouldGravGlovesOnehand()) {
		switch (twohandedact) {
		case ACT_VM_DRAW_EXTENDED:
			return ACT_VM_DRAW;
		case ACT_VM_IDLE_EXTENDED:
			return ACT_VM_IDLE;
		case ACT_VM_FIRE_EXTENDED:
			return ACT_VM_PRIMARYATTACK;
		case ACT_VM_HOLSTER_EXTENDED:
...
bool CVancePlayer::ShouldGravGlovesOnehand() {
	return m_fGglovesMainpulatorTargetMass <= 20.0f;
}
...
void CVancePlayer::PostThink()
{
	...
	if (m_eGglovesManipulator) {
		...
		CBaseHLCombatWeapon* physgunent = static_cast<CBaseHLCombatWeapon*>(m_eGglovesManipulator);
		if (physgunent) {
			physgunent->ItemPostFrame();
		}
	}

	if (m_eGglovesManipulator && m_bGglovesManipulatorHasAttached && m_PerformingGesture != GestureAction::GravGloving) {
		CBaseHLCombatWeapon* physgunent = static_cast<CBaseHLCombatWeapon*>(m_eGglovesManipulator);
		physgunent->SecondaryAttack();
	}
	...
	if (m_PerformingGesture != GestureAction::None && gpGlobals->curtime >= m_fGestureFinishTime && IsAlive())
	{
		switch ( m_PerformingGesture )
		{
			...
			case GestureAction::ThrowingGrenade:
				...
				break;
			case GestureAction::GravGloving:
				m_bOneHandedWeapon = false;
				if (GetActiveWeapon())
				{
					GetActiveWeapon()->m_fDoNotDisturb = 0.0f;
	...
	if (m_PerformingGesture == GestureAction::GravGloving) {
		GravGlovingTick();
	}
...
void CVancePlayer::Spawn()
{
	...
	CreateViewModel(VM_ONEHANDEDGUN);
	if ( CBaseViewModel *pOneHandedGunViewModel = GetViewModel(VM_ONEHANDEDGUN)) {
		pOneHandedGunViewModel->SetWeaponModel(V_ONEHANDEDWEAPON, NULL);
		pOneHandedGunViewModel->SendViewModelMatchingSequence(pOneHandedGunViewModel->SelectWeightedSequence(ACT_VM_HOLSTER));
	}
...
void CVancePlayer::PlayerUse()
{
	...
	if (pUseEntity)
	{
		...
		if (IsSuitEquipped() && (m_afButtonPressed & IN_USE))
		{
			...
			//also dont play sounds if the dead and decaying corpse of a gravity gun in the basement doesnt want us to
			if (!pUseEntity->MyNPCPointer() && m_PerformingGesture != GestureAction::GravGloving && !m_bGglovesManipulatorHasAttached && !m_bGglovesManipulatorHasTarget)
			{
	...
	else if ((m_afButtonPressed & IN_USE) && m_PerformingGesture != GestureAction::GravGloving && ((!m_bGglovesManipulatorHasAttached && !m_bGglovesManipulatorHasTarget) || !IsSuitEquipped()))
	{
	...
	//if the gravgloves are up theyll handle it
	if (m_PerformingGesture == GestureAction::GravGloving) {
		usedSomething = false;
	}
