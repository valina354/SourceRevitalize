...
public:
	...
	virtual Activity	GetCrouchWalkActivity() { return !m_bSemiAutoMode ? ACT_VM_CROUCHWALK : ACT_VM_CROUCHWALK_EXTENDED; }
...
void CWeaponResistanceGun::ItemHolsterFrame() {
	CVancePlayer* pPlayer = static_cast<CVancePlayer*>(GetOwner());
	if (pPlayer && pPlayer->m_bOneHandedWeapon.Get()) {
		return;
	}
...
{
	//if we are being used by the players onehanded mode we shouldnt be updating animations
	CVancePlayer* pPlayer = static_cast<CVancePlayer*>(GetOwner());
	if (pPlayer && pPlayer->m_bOneHandedWeapon.Get()) {
		return false;
	}
	...
	if ( m_bSemiAutoMode )
	{
		switch ( iActivity )
		{
			...
			case ACT_VM_SPRINT_ENTER:
				iActivity = ACT_VM_SPRINT_ENTER_EXTENDED;
				break;
			case ACT_VM_SPRINT_LEAVE:
				iActivity = ACT_VM_SPRINT_LEAVE_EXTENDED;
				break;
		}
	}
