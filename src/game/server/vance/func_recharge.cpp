...
#define ARMOR_USAGE				1
#define SUPER_ARMOR_USAGE		10
#ifdef VANCE
#define SUITLESS_USAGE			25
#endif
...
{ 
	...
	bool bIsSuperCharger = HasSpawnFlags( SF_CITADEL_RECHARGER );
	bool bActivatorHasSuit = ( ( CBasePlayer * )pActivator )->IsSuitEquipped();

	// Only usable if you have the HEV suit on
	if ( !bIsSuperCharger && !bActivatorHasSuit )
	{
	...
	CBasePlayer *player = ( CBasePlayer * ) m_hActivator.Get();

	// charge the player
	int nMaxArmor = 100;
	int nIncrementArmor = ARMOR_USAGE;
	if ( bIsSuperCharger )
	{
		nMaxArmor = sk_suitcharger_citadel_maxarmor.GetInt();
		nIncrementArmor = SUPER_ARMOR_USAGE;
...
#ifdef VANCE
			if ( !bActivatorHasSuit )
			{
				nIncrementArmor = SUITLESS_USAGE;
			}
#endif
		}
	}
	...
	if ( player->ArmorValue() < nMaxArmor )
	{
		UpdateJuice( m_iJuice - nIncrementArmor );

		if ( bActivatorHasSuit )
		{
			player->IncrementArmorValue( nIncrementArmor, nMaxArmor );
		}
	}
...
{ 
	...
	bool bIsSuperCharger = HasSpawnFlags( SF_CITADEL_RECHARGER );
	bool bActivatorHasSuit = pPlayer->IsSuitEquipped();
	...
	// Only usable if you have the HEV suit on
	if ( !bActivatorHasSuit && !bIsSuperCharger )
	{
		if ( m_flSoundTime <= gpGlobals->curtime )
		{
			m_flSoundTime = gpGlobals->curtime + 0.62;
	...
	if ( !bActivatorHasSuit || pPlayer->ArmorValue() >= nMaxArmor )
	{
		// Citadel charger must also be at max health
		if ( !bIsSuperCharger || ( bIsSuperCharger && pActivator->GetHealth() >= pActivator->GetMaxHealth() ) )
		{
