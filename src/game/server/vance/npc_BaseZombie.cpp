...
void CNPC_BaseZombie::TempRagdoll_Become(const CTakeDamageInfo& info, bool bUseLRURetirement)
{
	m_hTemp_Ragdoll = static_cast<CRagdollProp*>( CreateServerRagdoll(this, m_nForceBone, info, COLLISION_GROUP_INTERACTIVE_DEBRIS, bUseLRURetirement) );
	m_bIsTempRagdoll = true;

	TempRagdoll_Hide();
}

void CNPC_BaseZombie::TempRagdoll_Hide()
{
	SetThink(&CNPC_BaseZombie::TempRagdoll_Think);
	SetNextThink(gpGlobals->curtime + 2.0f);

	m_iTemp_MoveType = GetMoveType();
...
void CNPC_BaseZombie::TempRagdoll_Think()
{
	m_hTemp_Ragdoll->RemoveDeferred();

	RemoveEffects(EF_NODRAW);
	RemoveSolidFlags(FSOLID_NOT_SOLID);
	SetMoveType((MoveType_t)m_iTemp_MoveType);

	SetAbsOrigin(m_hTemp_Ragdoll->GetAbsOrigin());

	m_iTemp_StandUpActivity = TempRagdoll_GetStandUpActivity();
	SetActivity((Activity)m_iTemp_StandUpActivity); // HACKHACK: fixes stuttering when going back into an NPC from ragdoll

	SetThink(&CNPC_BaseZombie::CallNPCThink);
	SetNextThink(gpGlobals->curtime);
}

int CNPC_BaseZombie::TempRagdoll_GetStandUpActivity()
{
	int iChest = LookupAttachment("chest");
	Vector origin, forward, right, up;
	Vector gUp(0, 0, 1.0f);
	m_hTemp_Ragdoll->GetAttachment(iChest, origin, &forward, &right, &up);
...
AI_BEGIN_CUSTOM_NPC( base_zombie, CNPC_BaseZombie )
	...
	DECLARE_TASK( TASK_ZOMBIE_PLAY_STAND_UP_SEQUENCE )
	...
	DECLARE_CONDITION( COND_ZOMBIE_RAGDOLL )
	...
	DEFINE_SCHEDULE
	(
		SCHED_ZOMBIE_STAND_UP,

		"	Tasks"
		"		TASK_STOP_MOVING		0"
		"		TASK_RESET_ACTIVITY		0"
		"		TASK_ZOMBIE_PLAY_STAND_UP_SEQUENCE		0"
		""
		"	Interrupts"
		""
	)
