...
class CWeaponMP5 : public CVanceMachineGun
{
	DECLARE_CLASS( CWeaponMP5, CVanceMachineGun );
	DECLARE_SERVERCLASS();
	DECLARE_DATADESC();
	DECLARE_ACTTABLE();
public:
...
void CWeaponMP5::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, Vector &vecShootOrigin, Vector &vecShootDir )
{
	...
	FireBulletProjectiles(1, vecShootOrigin, vecShootDir, VECTOR_CONE_PRECALCULATED,
...
int CWeaponMP5::WeaponRangeAttack1Condition( float flDot, float flDist )
{
	if (m_iClip1 <= 0)
	{
