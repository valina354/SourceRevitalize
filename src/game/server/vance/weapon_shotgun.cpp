...
//	void WeaponIdle( void );
	...
	void DryFire( void );
	void PrimaryAttackActual(bool secondary);
...
void CWeaponShotgun::PrimaryAttackActual(bool secondary) {
	// Only the player fires this way so we can cast.
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
	if (!pPlayer)
		return;
	...
	if (secondary) {
		FireBulletProjectiles(sk_plr_num_shotgun_pellets.GetInt(), vecSrc, vecAiming, GetBulletSpread() * 1.6f, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 1, -1, -1, 0, NULL, false, true);
		pPlayer->ViewPunch(QAngle(random->RandomFloat(-6.0f, -3.0f), random->RandomFloat(-2.0f, 2.0f), 0.0f) * 1.3f);
	} else {
		FireBulletProjectiles(sk_plr_num_shotgun_pellets.GetInt(), vecSrc, vecAiming, GetBulletSpread()  * 0.8f, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 1, -1, -1, 0, NULL, true, true);
