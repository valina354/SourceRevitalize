...
void CWeaponAR2::ItemPostFrame( void )
{
	...
	if (m_bPelletReload == true && !m_bInReload)
	{
		SendWeaponAnim(ACT_VM_RELOAD);
		m_flNextPrimaryAttack = gpGlobals->curtime + GetViewModelSequenceDuration();
		m_bPelletReload = false;
	}
}
...
void CWeaponAR2::PrimaryAttack(void)
{
	...
	if (m_iClip1 == GetMaxClip1() / 2 + 1)
		m_bPelletReload = true;
...
bool CWeaponAR2::Reload( void )
{
	...
	Activity actReload = ACT_VM_RELOAD_EXTENDED;
	...
	return BaseClass::DefaultReload(GetMaxClip1(), GetMaxClip2(), actReload);
}
...
void CWeaponAR2::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
	...
	FireBulletProjectiles(1, vecShootOrigin, vecShootDir, VECTOR_CONE_PRECALCULATED, MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 2);
