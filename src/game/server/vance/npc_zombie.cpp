...
#include "vance/vance_shareddefs.h"
...
{
	if (HasCondition(COND_ZOMBIE_RAGDOLL))
	{
		return SCHED_ZOMBIE_STAND_UP;
	}
...
void CZombie::GatherConditions( void )
{
	...
	if (m_bIsTempRagdoll)
	{
		SetCondition(COND_ZOMBIE_RAGDOLL);
	}
}
...
void CZombie::StartTask( const Task_t *pTask )
{
	...
	case TASK_ZOMBIE_PLAY_STAND_UP_SEQUENCE:
		{
			SetIdealActivity((Activity)m_iTemp_StandUpActivity);
			break;
		}
