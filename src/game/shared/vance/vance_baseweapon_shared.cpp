...
void CBaseVanceWeapon::Precache(void)
{
	...
	const char *weapon_mag_model = GetVanceWpnData().szWeaponMag;
	if ( V_strlen( weapon_mag_model ) != 0 )
	{
		PrecacheModel( weapon_mag_model );
	}
...
bool CBaseVanceWeapon::CanCrouchWalkBob()
{
	CVancePlayer *pPlayer = assert_cast<CVancePlayer *>(GetOwner());
	if (pPlayer && SelectWeightedSequence(ACT_VM_CROUCHWALK) == ACTIVITY_NOT_AVAILABLE)
		return false;
...
void CBaseVanceWeapon::WeaponIdle(void)
{
	...
	bool stillinsprintenter = false;

	if (pPlayer->IsSprinting() && speed >= 300 && (pPlayer->m_nButtons & IN_SPEED))
	{
		...
		if (HasWeaponIdleTimeElapsed() || 
				(
					...
					(
						...
						GetActivity() == GetCrouchWalkActivity() ||
			...
			// (idling || not kicking && activity == idle, walk, crouchwalk or lowered || 
		{
		...
		{
			//try to do the sprint intro anim
			if (!m_bSprinting) {
				m_fSprintIntroEndTime = 0.0f;
				SendWeaponAnim(ACT_VM_SPRINT_ENTER);
				if (GetActivity() == ACT_VM_SPRINT_ENTER || GetActivity() == ACT_VM_SPRINT_ENTER_EXTENDED) {
					m_fSprintIntroEndTime = gpGlobals->curtime + SequenceDuration();
				}
				m_bSprinting = true;
			}

			if (m_bSprinting && m_fSprintIntroEndTime <= gpGlobals->curtime) {
				SendWeaponAnim(iActivity);
			} else {
				stillinsprintenter = true;
			}
		}
	}
	...
	else if (!m_bSprinting && CanCrouchWalkBob() && speed >= 30 && pPlayer->GetWaterLevel() != 3 && (pPlayer->GetFlags() & FL_ONGROUND) && (pPlayer->GetFlags() & FL_DUCKING || pPlayer->m_Local.m_bDucking) && !(pPlayer->GetFlags() & FL_DUCKING && pPlayer->m_Local.m_bDucking))
		 
	{
		if (GetActivity() != GetCrouchWalkActivity() && GetActivity() != ACT_VM_KICK && (GetActivity() == GetIdleActivity() || GetActivity() == GetSprintActivity()))
		{
			SendWeaponAnim(GetCrouchWalkActivity());
		}
		else if (HasWeaponIdleTimeElapsed())
		{
			SendWeaponAnim(GetCrouchWalkActivity());
	...
	else if (!m_bSprinting && CanWalkBob() && speed >= 100 && pPlayer->GetWaterLevel() != 3 && (pPlayer->GetFlags() & FL_ONGROUND))
	{
	...
	else
	{
		...
		else if ((speed <= 300 || !(pPlayer->m_nButtons & IN_SPEED)) && (GetActivity() == GetSprintActivity() || GetActivity() == ACT_VM_SPRINT_ENTER || GetActivity() == ACT_VM_SPRINT_ENTER_EXTENDED
			|| GetActivity() == ACT_VM_SPRINT_LEAVE || GetActivity() == ACT_VM_SPRINT_LEAVE_EXTENDED
			|| GetActivity() == ACT_VM_SPRINT2 || GetActivity() == ACT_VM_SPRINT2_EXTENDED))
		{
			if (m_bSprinting && stillinsprintenter) {
				m_fSprintExitEndTime = 0.0f;
				m_bSprinting = false;
			}
			if (m_bSprinting) {
				m_fSprintExitEndTime = 0.0f;
				SendWeaponAnim(ACT_VM_SPRINT_LEAVE);
				if (GetActivity() == ACT_VM_SPRINT_LEAVE || GetActivity() == ACT_VM_SPRINT_LEAVE_EXTENDED) {
					m_fSprintExitEndTime = gpGlobals->curtime + SequenceDuration();
				}
				m_bSprinting = false;
			}
			if (!m_bSprinting && m_fSprintExitEndTime <= gpGlobals->curtime) {
				SendWeaponAnim(GetIdleActivity());
			}
		}
		...
		else if (speed <= 30 && GetActivity() == GetCrouchWalkActivity())
		{
			SendWeaponAnim(GetIdleActivity());
		}
		else if (HasWeaponIdleTimeElapsed())
