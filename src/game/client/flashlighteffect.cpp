...
public:
	...
	virtual bool ShouldHitEntity( IHandleEntity *pServerEntity, int contentsMask )
	{
		...
		if ( ( (C_BaseViewModel *)pEntity != NULL ) ||
			 pEntity == m_pPlayer ||
...
void CFlashlightEffect::UpdateMuzzleFlashTexture(const char *pTextureName)
{
	static const char* pEmptyString = "";
	...
	if (!m_MuzzleFlashTexture.IsValid() ||
		V_stricmp(m_textureName, pTextureName) != 0)
	{
		if (pTextureName == pEmptyString)
		{
		...
		{
			m_MuzzleFlashTexture.Init(pTextureName, TEXTURE_GROUP_OTHER, true);
		}
		V_strncpy(m_textureName, pTextureName, sizeof(m_textureName));
	}
}
