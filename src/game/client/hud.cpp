...
{
	m_clrNormal = scheme->GetColor("Normal", Color(255, 0, 0, 255));
...
void CHud::DrawSelfCropped_VANCESpecific(int x, int y, int cropx, int cropy, int cropw, float croph, wchar_t cCharacterInFont, Color clr, vgui::HFont hudFont)
{
	vgui::surface()->DrawSetTextFont(hudFont);
	vgui::surface()->DrawSetTextColor(clr);

	//This is specifically for the new hud. Do NOT use for other uses unless you know what you are doing!

	int height = vgui::surface()->GetFontTall(hudFont);
	float frac = (height - croph) / (float)height;
