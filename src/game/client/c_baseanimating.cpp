...
void C_BaseAnimating::DispatchMagThrow( const char *options )
{
	...
	C_BaseVanceWeapon *pWeapon = (C_BaseVanceWeapon *)GetActiveWeapon();
	if ( pWeapon )
	{
		pModel = (char *)pWeapon->GetVanceWpnData().szWeaponMag;
	}
	...
	if ( pGib )
	{
		...
		int m_iBodyGroup = pGib->FindBodygroupByName( "bullet" );
		if (m_iBodyGroup != -1)
		{
			pGib->SetBodygroup( m_iBodyGroup, 1 );
		}
	}
}
#endif // VANCE
