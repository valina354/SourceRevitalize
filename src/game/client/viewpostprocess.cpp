...
static ConVar mat_dynamic_tonemapping( "mat_dynamic_tonemapping", "0", FCVAR_CHEAT );
										...
										int x, int y, int w, int h )
{
	...
	pRenderContext->PushRenderTargetAndViewport();
	ITexture *pSrc = materials->FindTexture( "_rt_VanceHDR", TEXTURE_GROUP_RENDER_TARGET );
...
static void DoPreBloomTonemapping( IMatRenderContext *pRenderContext, int nX, int nY, int nWidth, int nHeight, float flAutoExposureMin, float flAutoExposureMax )
{
	...
	{
		SetToneMapScale(pRenderContext, 1.0f, flAutoExposureMin, flAutoExposureMax);
	}
}
...
bool DoSunAndGlowEffects( int x, int y, int w, int h )
{
	...
	if ( C_Sun::Get() /*&& engine->IsSkyboxVisibleFromPoint(CurrentViewOrigin())*/ )
	{
		...
		DF_SetMaterialVarVector( s_pSunRaysMaterial, "$suncolor",
								 !modulate && alpha > 0 ? thesun->m_vOverlayColor * ( 1.f - alpha / 255.f ) : thesun->m_vOverlayColor );
		...
		if ( r_suneffects_blur.GetBool() )
		{
			...
			SetRenderTargetAndViewPort( dest_rt1 );
			pRenderContext->DrawScreenSpaceRectangle( xblurMat, 0, 0, nMaskWidth, nMaskHeight, 0, 0, dest_rt1->GetActualWidth() - 1,
													  dest_rt1->GetActualHeight() - 1, dest_rt1->GetActualWidth(),
													  dest_rt1->GetActualHeight() );

			//Gaussian blur y srt1 to srt0
			SetRenderTargetAndViewPort( dest_rt0 );
			pRenderContext->DrawScreenSpaceRectangle( yblurMat, 0, 0, nMaskWidth, nMaskHeight, 0, 0, dest_rt0->GetActualWidth() - 1,
													  dest_rt0->GetActualHeight() - 1, dest_rt0->GetActualWidth(),
													  dest_rt0->GetActualHeight() );
		}
