...
void CHudSuitPower::Paint()
{
	...
	// get bar chunks (VANCE: Changing Meter from chunks to a solid bar)
	/*int chunkCount = m_flBarWidth / (m_flBarChunkWidth + m_flBarChunkGap);
	int enabledChunks = (int)((float)chunkCount * (m_flSuitPower * 1.0f/100.0f) + 0.5f );*/

	// see if we've changed power state
	int lowPower = 0;
	//if (enabledChunks <= (chunkCount / 4)) (VANCE: Changing Meter from chunks to a solid bar)
	...
	float flPerc = RemapVal(m_flSuitPower, 0.0f, 100.0f, 0.0f, 1.0f);

	surface()->DrawFilledRect(xpos, ypos, xpos + (m_flBarWidth * flPerc), ypos + m_flBarHeight);
