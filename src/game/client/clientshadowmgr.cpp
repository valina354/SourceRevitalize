...
bool CClientShadowMgr::Init()
{
	...
	bool bShadowrendertotexture = false; //r_shadowrendertotexture.GetBool();
	if (!bLowEnd && bShadowrendertotexture)
	{
...
void CClientShadowMgr::InitDepthTextureShadows()
{
	...
	if (!m_bDepthTextureActive)
	{
		...
		// using RT_SIZE_NO_CHANGE allows us to resize the projected texture past the depth buffer
		...
		const int iCadcadedShadowRes = 2048;
		m_CascadedColorTexture.InitRenderTarget(iCadcadedShadowRes * 4, iCadcadedShadowRes, RT_SIZE_NO_CHANGE,
			nullFormat, MATERIAL_RT_DEPTH_NONE, false, "_rt_CascadedShadowColor");
		m_CascadedDepthTexture.InitRenderTarget(iCadcadedShadowRes * 4, iCadcadedShadowRes, RT_SIZE_NO_CHANGE,
...
void CClientShadowMgr::PreRender()
{
	...
	bool bRenderToTextureActive = false;// r_shadowrendertotexture.GetBool();
