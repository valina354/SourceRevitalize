...
#ifdef DXVK
ConVar ae_lensflare_intensity( "ae_lensflare_intensity", "0.35", FCVAR_ARCHIVE );
#else
...
SHADER_DRAW
{
	SHADOW_STATE
	{
		...
		{
			DECLARE_STATIC_PIXEL_SHADER( ae_lensflare_ps30 );
			SET_STATIC_PIXEL_SHADER( ae_lensflare_ps30 );
		}
		else
		{
			DECLARE_STATIC_PIXEL_SHADER( ae_lensflare_ps30 );
	...
	DYNAMIC_STATE
	{
		...
		if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
		{
			DECLARE_DYNAMIC_PIXEL_SHADER( ae_lensflare_ps30 );
			SET_DYNAMIC_PIXEL_SHADER( ae_lensflare_ps30 );
		}
		else
		{
			DECLARE_DYNAMIC_PIXEL_SHADER( ae_lensflare_ps30 );
