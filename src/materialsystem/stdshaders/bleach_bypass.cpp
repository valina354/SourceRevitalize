...
#include "bleach_bypass_ps30.inc"
...
BEGIN_VS_SHADER_FLAGS( Bleach_Bypass, "Help for Bleach Bypass", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_PIXEL_SHADER( bleach_bypass_ps30 );
				SET_STATIC_PIXEL_SHADER( bleach_bypass_ps30 );
			}
			else
			{
				DECLARE_STATIC_PIXEL_SHADER( bleach_bypass_ps30 );
		...
		DYNAMIC_STATE
		{
			...
			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( bleach_bypass_ps30 );
				SET_DYNAMIC_PIXEL_SHADER( bleach_bypass_ps30 );
			}
			else
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( bleach_bypass_ps30 );
