...
BEGIN_VS_SHADER_FLAGS( Complements, "Help for Complements", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_PIXEL_SHADER( complements_ps30 );
			SET_STATIC_PIXEL_SHADER( complements_ps30 );
		}
		...
		DYNAMIC_STATE
		{
			...
			DECLARE_DYNAMIC_PIXEL_SHADER( complements_ps30 );
			SET_DYNAMIC_PIXEL_SHADER( complements_ps30 );
		}
		Draw();
	}
END_SHADER