										...
										IShaderShadow* pShaderShadow, DrawFlashlight_dx90_Vars_t &vars )
{
	...
	bool bRoughness = vars.m_nRoughness != -1 && params[vars.m_nRoughness]->IsTexture();
	bool bMetallic = vars.m_nMetallic != -1 && params[vars.m_nMetallic]->IsTexture();
	...
	if( pShaderShadow )
	{
		...
		pShaderShadow->EnableTexture(SHADER_SAMPLER9, true);				// roughness sampler
		pShaderShadow->EnableTexture(SHADER_SAMPLER10, true);				// metallic sampler
		...
		pshIndex.SetNORMALMAP(vars.m_bBump ? 1 : 0);
	...
	else
	{
		...
		if (bRoughness)
		{
			BindTexture(SHADER_SAMPLER9, vars.m_nRoughness);
		}
		else
		{
		...
		if (bMetallic)
		{
			BindTexture(SHADER_SAMPLER10, vars.m_nRoughness);
		}
		else
		{
		...
		pos[2] = flashlightState.m_vecLightOrigin[2];
		pos[3] = 1.0f / ((0.6f * flashlightState.m_FarZ) - flashlightState.m_FarZ);		// DX8 needs this
		pShaderAPI->SetPixelShaderConstant(PSREG_FRESNEL_SPEC_PARAMS, pos, 1);
