...
BEGIN_VS_SHADER( GaussianX, "Help for Gaussian X" )
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_PIXEL_SHADER( gaussianx_ps30 );
				SET_STATIC_PIXEL_SHADER( gaussianx_ps30 );
			}
		}
		...
		DYNAMIC_STATE
		{
			...
			//if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_DYNAMIC_PIXEL_SHADER( gaussianx_ps30 );
				SET_DYNAMIC_PIXEL_SHADER( gaussianx_ps30 );
			}
		}
		Draw();
	}
