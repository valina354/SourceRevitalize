...

{
	...
	else // not snapshotting -- begin dynamic state
	{
		...
		if (pCascadedDepthTexture)
		{
			pShader->BindTexture(SHADER_SAMPLER8, pCascadedDepthTexture, -1);
		...
		pShaderAPI->SetPixelShaderConstant(10, csmFwd.Base());

		Vector csmLight = csmData.light.AsVector3D();
		pShaderAPI->SetPixelShaderConstant(11, csmLight.Base());

		Vector csmAmbient = csmData.ambient.AsVector3D();
		pShaderAPI->SetPixelShaderConstant(12, csmAmbient.Base());

		float biasVar[2] = { r_csm_slopescalebias.GetFloat(), r_csm_bias.GetFloat() };
		pShaderAPI->SetPixelShaderConstant(13, biasVar);

		float textureSize[2] = { pCascadedDepthTexture->GetActualWidth(), pCascadedDepthTexture->GetActualHeight() };
		pShaderAPI->SetPixelShaderConstant(14, textureSize);

		pShaderAPI->SetPixelShaderConstant(15, GetDeferredExt()->GetLightData_Global().sizes.Base());
	}
	pShader->Draw();
}
