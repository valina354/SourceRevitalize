...
/*^*/ // 		printf("\t\t[2] snapshotting...\n");
			...
			if ( bHasFlashlight )
			{
				...
				if ( !IsX360() )
				{
					if (bHasBaseTexture && params[info.m_nBaseTexture]->IsTexture())
					{
...
/*^*/ // 	printf("\t\t[%1d] VERTEX_NORMAL\n",(flags&VERTEX_NORMAL)!=0);
			...
			if ( bHasBaseTexture )
			{
				...
				{
					ITexture *pBaseTexture = bHasBaseTexture ? params[info.m_nBaseTexture]->GetTextureValue() : NULL;
...
/*^*/ // 		printf("\t\t      -> CShaderShadowDX8::VertexShaderVertexFormat( flags=%08x, texcount=%d )\n",flags,nTexCoordCount);
			...
			else // !(bHasBump || bHasDiffuseWarp)
			{
				...
				bool bDoDepthBlend = IsBoolSet( info.m_nDepthBlend, params );
...
//				SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
			...
			SET_DYNAMIC_VERTEX_SHADER_COMBO(COMPRESSED_VERTS, (int)vertexCompression);
			SET_DYNAMIC_VERTEX_SHADER_COMBO(MORPHING, pShaderAPI->IsHWMorphingEnabled());
			SET_DYNAMIC_VERTEX_SHADER_CMD(DynamicCmdsOut, vertexlit_and_unlit_generic_bump_vs30);
