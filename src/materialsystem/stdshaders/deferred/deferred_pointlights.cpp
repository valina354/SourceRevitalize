...
#include "deferred_pointlights_ps30.inc"

#include "IDeferredExt.h"

BEGIN_VS_SHADER_FLAGS(deferred_pointlights, "Help for Bloom", SHADER_NOT_EDITABLE)
BEGIN_SHADER_PARAMS
...
SHADER_DRAW
{
	SHADOW_STATE
	{
		...
		{
			DECLARE_STATIC_PIXEL_SHADER(deferred_pointlights_ps30);
			SET_STATIC_PIXEL_SHADER(deferred_pointlights_ps30);
		}
	}
	...
	DYNAMIC_STATE
	{
		...
		Vector4D lightData[MAX_LIGHTS_SIMPLE * 2];
		for (int i = 0; i < MAX_LIGHTS_SIMPLE * 2; i = i + 2)
		{
			lightData_Simple_t light = GetDeferredExt()->GetLightData_Simple(i / 2);
		...
		pShaderAPI->SetPixelShaderConstant(4, lightData[0].Base(), MAX_LIGHTS_SIMPLE * 2);


		DECLARE_DYNAMIC_VERTEX_SHADER(deferred_composite_vs30);
		SET_DYNAMIC_VERTEX_SHADER(deferred_composite_vs30);

		DECLARE_DYNAMIC_PIXEL_SHADER(deferred_pointlights_ps30);
		SET_DYNAMIC_PIXEL_SHADER(deferred_pointlights_ps30);
	}
	Draw();
}
END_SHADER
