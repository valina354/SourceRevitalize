...
#include "deferred_spotlight_shadow_ps30.inc"

#include "IDeferredExt.h"

BEGIN_VS_SHADER_FLAGS(deferred_spotlightshadow, "", SHADER_NOT_EDITABLE)
BEGIN_SHADER_PARAMS
...
SHADER_DRAW
{
	SHADOW_STATE
	{
		...
		pShaderShadow->EnableTexture(SHADER_SAMPLER7, true); // shadow depth starts here
		...
		{
			DECLARE_STATIC_PIXEL_SHADER(deferred_spotlight_shadow_ps30);
			SET_STATIC_PIXEL_SHADER(deferred_spotlight_shadow_ps30);
		}
	}
	...
	DYNAMIC_STATE
	{
		...
		Vector4D lightData[MAX_LIGHTS_SPOTSHADOW * 7]; // oof, this is big
		for (int i = 0; i < MAX_LIGHTS_SPOTSHADOW * 7; i = i + 7)
		{
			lightData_Spot_Shadow_t light = GetDeferredExt()->GetLightData_SpotShadow(i / 7);
			...
			lightData[i + 3] = light.projection[0];
			lightData[i + 4] = light.projection[1];
			lightData[i + 5] = light.projection[2];
			lightData[i + 6] = light.projection[3];

			BindTexture((Sampler_t)(SHADER_SAMPLER7 + (i / 7)), light.depth, -1);
		}
		pShaderAPI->SetPixelShaderConstant(4, lightData[0].Base(), MAX_LIGHTS_SPOTSHADOW * 7);

		DECLARE_DYNAMIC_VERTEX_SHADER(deferred_composite_vs30);
		SET_DYNAMIC_VERTEX_SHADER(deferred_composite_vs30);

		DECLARE_DYNAMIC_PIXEL_SHADER(deferred_spotlight_shadow_ps30);
		SET_DYNAMIC_PIXEL_SHADER(deferred_spotlight_shadow_ps30);
	}
	Draw();
}
END_SHADER
